                                         ...
                                         MTKTextureLoader* textureLoader)
{
    ...
    for (MDLMaterialProperty *property in propertiesWithSemantic)
    {
           ...
           property.type == MDLMaterialPropertyTypeURL)
        {
            ...
            CHA_ASSERT( !err, "Error loading texture:", property.URLValue );
                               ...
                               NS::Error** pError)
{
    ...
    CHA_ASSERT( !err, "Error loading MTKMesh" );
    if (pError && err)
    {
                                          ...
                                          NS::Error** pError)
{
    ...
    CHA_ASSERT( modelFileURL, "Could not find model file in bundle: ", modelFileURL.absoluteString.UTF8String );
    ...
    CHA_ASSERT( asset, "Failed to open model file with given URL:", modelFileURL.absoluteString.UTF8String );
    ...
    CHA_ASSERT_NULL_ERROR( pInternalError, "Error loading model:" );
...
MTL::Texture* newTextureFromCatalog( MTL::Device* pDevice, const char* name, MTL::StorageMode storageMode, MTL::TextureUsage usage )
{
    ...
    CHA_ASSERT( !err, "Error loading texture:", name );
